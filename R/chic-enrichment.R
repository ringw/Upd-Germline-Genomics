...
chic_macs_test <- function(
  input_track, input_track_gaussian_smooth, mark_tracks
) {
  diameter <- attr(mark_tracks[[1]], "diameter")
  poisson_param <- mark_tracks %>%
    sapply(
      \(tr) 1 / 1000 / 1000 / 1000 * attr(tr, "num_tags") * diameter
      * input_track
    )
  poisson_ll <- mapply(
    \(input, mark) mapply(
      # Converting Rle to numeric is a bit wasteful, but efficient (<1 sec).
      # This allows us to extend peaks to the size of the sliding window in bp
      # (diameter) using rollmax.
      \(input_chr, mark_chr) ppois(
        floor(mark_chr) %>% as.numeric, input_chr %>% as.numeric,
        lower.tail=F, log.p=T
      ) %>%
        # min poisson p-value
        zoo::rollapply(width = diameter, min, fill = rep("extend", 3)) %>%
        as("Rle"),
      input,
      mark
    ) %>%
...
chic_macs_generate_table <- function(p_track, p_threshold = 1e-5) {
  grouped_df <- (p_track < p_threshold) %>%
    mapply(
