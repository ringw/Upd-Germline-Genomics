...
repli <- tar_read(repli.timing_S2_chr)
...
H3K4 <- read.table("illustrations/GSE20787_repset.3211336.smoothedM.bedgraph.gz", sep=" ", skip=1) %$%
  GRanges(V1, IRanges(V2 + 1, V3), score = V4) %>%
  lift_dm3_to_flybase_dm6() %>%
  time_dm6()
dm6_reference <- H3K4
H3K27 <- read.table("illustrations/GSE20781_repset.4621697.smoothedM.bedgraph.gz", sep=" ", skip=1) %$%
  GRanges(V1, IRanges(V2 + 1, V3), score = V4) %>%
  lift_dm3_to_flybase_dm6() %>%
  time_dm6()
H3K9 <- read.table("illustrations/GSE20794_repset.3506194.smoothedM.bedgraph.gz", sep=" ", skip=1) %$%
  GRanges(V1, IRanges(V2 + 1, V3), score = V4) %>%
  lift_dm3_to_flybase_dm6() %>%
  time_dm6()
...
cairo_pdf("figure/Repli-CHIP-chip-S2.pdf", w = 6, h = 4.5)
print(plot_repli_track_raster(proj, log2_limits = NULL) + labs(fill = "y"))
dev.off()
